...
const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-10 flex items-center gap-4 px-6 h-16 bg-white/90 backdrop-blur-md border-b border-slate-200 flex-shrink-0">
      {/* Hamburger — mobile */}
      <button
        onClick={onMenuClick}
        className="lg:hidden p-2 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all duration-150"
      ...
      <div className={[
        ...
        <span className="absolute right-3 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-1">
          <kbd className="text-[10px] text-slate-400 bg-slate-200 border border-slate-300 rounded px-1 py-0.5 font-mono leading-none">
            ⌘K
          </kbd>
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 ml-auto">
        ...
        {/* Theme toggle */}
        <button
          className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all duration-150"
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="relative p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all duration-150"
          ...
          {notifOpen && (
            <div className="absolute right-0 top-[calc(100%+8px)] w-96 bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-900/8 animate-fade-in-down overflow-hidden z-50">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
                <div>
                  <h3 className="text-sm font-semibold text-slate-800">Notifications</h3>
                  <p className="text-xs text-slate-400 mt-0.5">{unread} unread alerts</p>
                </div>
                ...
                {notifications.map((n) => (
                  <li
                    ...
                    className={[
                      ...
                      n.read ? 'hover:bg-slate-50' : 'bg-amber-50/40 hover:bg-amber-50/70',
                    ].join(' ')}
                  >
        ...
        <div ref={profileRef} className="relative">
          <button
            ...
            className="flex items-center gap-2.5 pl-1 pr-2.5 py-1 rounded-xl hover:bg-slate-100 transition-all duration-150 group"
          ...
          >
            ...
            <div className="hidden sm:block text-left">
              <p className="text-xs font-semibold text-slate-700 leading-none">James Dela Cruz</p>
              <p className="text-[10px] text-slate-400 mt-0.5">Fleet Administrator</p>
            </div>
            <ChevronDown
              size={14}
              className={[
          ...
          {/* Profile dropdown */}
          {profileOpen && (
            <div className="absolute right-0 top-[calc(100%+8px)] w-60 bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-900/8 animate-fade-in-down overflow-hidden z-50">
              <div className="px-5 py-4 border-b border-slate-100 bg-gradient-to-br from-slate-50 to-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-800">James Dela Cruz</p>
              ...
              <ul className="py-1.5">
                {[
                  ...
                  <li key={item.label}>
                    <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors duration-100">
